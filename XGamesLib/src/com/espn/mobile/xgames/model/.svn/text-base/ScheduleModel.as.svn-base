...
package com.espn.mobile.xgames.model
{
	 ...
	 */	
	public class ScheduleModel extends BaseService
	{
		/**
		 * IEventDispatcher.
		 */	
		 ...
		 */
		public function getScheduleList(url:String, isEvent:Boolean):void
		{
			try
			{
				isInPersonSchedule = isEvent;
		 ...
		 * the schedule object
		 */
		private function parseSchedule(isEvent:Boolean, itemsArray:Array):void {
		 ...
		 */
		public function getScheduleData(filterParam:String, filterParamValue:String): ArrayCollection {
			
			var activeScheduleCollection:ArrayCollection = new ArrayCollection();
			var tempScheduleVO:ScheduleVO = scheduleVO;
			
			if(filterParam.toLowerCase() == FILTER_BY_EVENT && scheduleVO_event) {
				
				tempScheduleVO = scheduleVO_event;
				activeScheduleCollection = setEventList(activeScheduleCollection, tempScheduleVO, filterParamValue, FILTER_BY_EVENT);
		 ...
		 */
		private function setSubCategoryList(isEvent:Boolean):void {
			...
			if(isEvent && scheduleVO_event && scheduleVO_event.eventList) {
				
				scheduleSubCategoryList_event = new ArrayCollection();
				...
				for(var count:Number = 0; count < eventList.length; count++) {						
					...
					if(!itemDictionary[scheduleItemVO.date]) {
						scheduleSubCategoryList_event.addItem(item);
				...
				scheduleSubCategoryList = new ArrayCollection();
				...
				for(var count_event:Number = 0; count_event < eventList.length; count_event++) {						
					...
					if(!itemDictionary[scheduleItemVO.date]) {
						scheduleSubCategoryList.addItem(item);
						itemDictionary[scheduleItemVO.date] = item;	
					}					
